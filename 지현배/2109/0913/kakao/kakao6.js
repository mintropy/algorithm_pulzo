function solution(board, skill) {
  ...
  let arr = Array(N + 1).fill().map(_ => Array(M + 1).fill(0));
  skill.map(e => {
      let [type, r1, c1, r2, c2, degree] = e;
      arr[r1][c1] += degree * (type == 1 ? -1 : 1);
      arr[r1][c2 + 1] += degree * (type == 1 ? 1 : -1);
      arr[r2 + 1][c1] += degree * (type == 1 ? 1 : -1);
      arr[r2 + 1][c2 + 1] += degree * (type == 1 ? -1 : 1);
  });
  for (let i = 0; i < N; i++) {
      let cnt = 0;
      for (let j = 0; j < M; j++) {
...
/*function solution(board, skill) {
  ...
  let N = board.length;
  let M = board[0].length
  let arr = Array(N).fill().map(_ => []);
  skill.map(e => {
      let [type, r1, c1, r2, c2, degree] = e;
      for (let i = r1; i <= r2; i++) {
          arr[i].push([c1, degree * (type == 1 ? -1 : 1)]);
          arr[i].push([c2 + 1, degree * (type == 1 ? 1 : -1)]);
      }
  });
  for (let i = 0; i < N; i++) {
      arr[i].sort((a, b) => b[0] - a[0]);
  }
  for (let i = 0; i < N; i++) {
      let cnt = 0;
      for (let j = 0; j < M; j++) {
          while (arr[i].length > 0 && arr[i][arr[i].length - 1][0] == j) {
              cnt += arr[i].pop()[1];
          }
          board[i][j] += cnt;
          if (board[i][j] > 0) answer++;
